...
hintlist <- function() {
  ...
  hints[next.hint()] <- 'the function will take the factor as its only argument'
  hints[next.hint()] <- 'it will have a return value'
  hints[next.hint()] <- 'the return call goes last'
  next.exercise(2)  # count number of zeros in vector
  hints[next.hint()] <- 'the function will take v as argument'
  hints[next.hint()] <- 'it will return a single integer'
  next.exercise(3)  # cont number of negative, positive, and zeros in vector
  hints[next.hint()] <- 'count the number of negative, positive and zero values any way you choose'
  hints[next.hint()] <- 'remember, only one object can be returned'
  hints[next.hint()] <- 'wrap the three numbers in a vector or list'
  hints[next.hint()] <- 'remember to create a named object'
  next.exercise(4)  # remove columns from data frame
  hints[next.hint()] <- 'this function will take two arguments: the data frame and the column specs'
  ...
  hints[next.hint()] <- 'recall how you can refer to an object with its name as a string'
  hints[next.hint()] <- 'use get'
  next.exercise(6)  # function to select random rows from data frame
  hints[next.hint()] <- 'use the sample function to get random numbers'
  hints[next.hint()] <- 'use the numeric indices generated by sample to select random rows'
  next.exercise(7)  # warpbreaks exercise with functions
  hints[next.hint()] <- 'split warpbreaks into a list by the values of tension'
  hints[next.hint()] <- 'this can be done with the split function'
  hints[next.hint()] <- 'apply the function from exercise 6 to this list'
  hints[next.hint()] <- 'you can use lapply to do it'
  hints[next.hint()] <- 'put the subsets together'
  hints[next.hint()] <- 'this can be done by sequential calls to rbind'
  hints[next.hint()] <- 'a loop would do it'
  ...
  hints[next.hint()] <- 'get all the objects in datasets into a list'
  hints[next.hint()] <- 'think how you can get the names of all objects in an environment'
  hints[next.hint()] <- 'use ls'
  hints[next.hint()] <- 'you now have a character vector of names'
  hints[next.hint()] <- 'think how you can refer to an object with its name as a string'
